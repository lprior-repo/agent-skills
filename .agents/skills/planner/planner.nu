...
# Creation: bd create (persists to database)
#
...
# Detect current repository context - repo root, name, and .beads/ paths
def detect-repo-context []: nothing -> record {
  ...
  let repo_name = try {
    ...
    let result = (^git remote get-url origin | complete)
    if $result.exit_code == 0 {
      let remote_url = ($result.stdout | str trim)
      if ($remote_url | is-empty) {
        $repo_root | path basename
      } else {
        $remote_url | path basename | str replace '.git' ''
      }
    } else {
  ...
  let beads_dir = $"($repo_root)/.beads"
  let template_path = $"($beads_dir)/BEAD_TEMPLATE.md"
  let schema_dir = $"($beads_dir)/schemas"
...
# Ensure .beads/ directory structure exists in current repo
def ensure-beads-directory [] {
  let context = (detect-repo-context)
  
  if not ($context.beads_dir | path exists) {
    mkdir $context.beads_dir
    print $"✅ Created .beads/ directory in ($context.repo_root)"
  }
  
  if not ($context.schema_dir | path exists) {
    mkdir $context.schema_dir
    print $"✅ Created schemas/ directory in ($context.schema_dir)"
  }
  
  # Check if template exists, warn if not
  if not ($context.template_path | path exists) {
    print $"⚠️  Warning: BEAD_TEMPLATE.md not found at ($context.template_path)"
    print "   You may need to create a template or copy from another repo"
  }
}
...
def check-required-commands [] {
  ...
  if (which bd | is-empty) {
    error make {msg: "Required command 'bd' not found. Ensure beads is installed."}
  }
}
...
def check-required-files [] {
  ...
  # Ensure .beads/ structure exists
  ensure-beads-directory
  
  # Check if template exists (warn but don't fail - user might create it)
  if not ($context.template_path | path exists) {
    print $"⚠️  Warning: Template not found at ($context.template_path)"
    print "   Create a BEAD_TEMPLATE.md or the script will use fallback"
  }
...
# Get beads schema directory (from env or detect from repo)
def get-beads-schema-dir []: nothing -> string {
  if ($env.BEADS_SCHEMA_DIR? != null) {
    $env.BEADS_SCHEMA_DIR | path expand
  } else {
    let context = (detect-repo-context)
    $context.schema_dir
  }
...
($bead.yaml)"
  ...
  # Write YAML to temp file for bd create (native nushell temp path)
  let temp_base = if ($env.TMPDIR? != null) { $env.TMPDIR } else if ($env.TEMP? != null) { $env.TEMP } else { "/tmp" }
  ...
  let creation_result = try {
    ...
    # Check file size (bd might have limits)
    ...
    let result = (do {
      ^bd create --title $task.title -t $task.type -p $task.priority -d $bead_desc
    } | complete)

    if $result.exit_code == 0 {
      # Extract bead ID from output if possible
      let bd_id = try { $result.stdout | str trim } catch { "unknown" }
      ...
      save-session $session
      print $"✅ Bead created in bd: ($bead_id) -> ($bd_id)"
      "success"
    } else {
...
def "main process" [
  ...
  print ""
  print "Phase 3: Creating beads in bd..."
  ...
  if $session.summary.created > 0 {
    ...
    for bead_id in ($session.beads | columns) {
      ...
      if $bead.status == "CREATED" {
        let task = ($session.tasks | get $bead.task_id)
        print $"  • ($bead.bd_id? | default $bead_id): ($task.title)"
      }
    }
    print ""
  }
...
def expand-template [task_data: record, bead_id: string] {
  ...
  let inversions_cue = if ($all_inversions | length) > 0 {
    ...
    $"    [\n      {failure: \"Implementation incomplete\", prevention: \"Follow implementation tasks in phase_2\", test_for_it: \"test_implementation_complete\"}\n    ]"
  }
  ...
  let happy_cue = if ($happy_tests | length) > 0 {
    let lines = ($happy_tests | each { |t|
      $'{name: \"test_($t | str replace -a " " "_" | str replace -a "(" "" | str replace -a ")" "" | str replace -a ":" "" | str downcase | str substring 0..<80)\", given: \"($description)\", when: \"($t)\", then: [\"Behavior verified\"], real_input: \"See task specification\", expected_output: \"See task specification\"}'
    } | str join ',\n      ')
    $"    [\n      ($lines)\n    ]"
  } else {
  ...
  let error_cue = if ($error_tests | length) > 0 {
    let lines = ($error_tests | each { |t|
      $'{name: \"test_($t | str replace -a " " "_" | str replace -a "(" "" | str replace -a ")" "" | str replace -a ":" "" | str downcase | str substring 0..<80)\", given: \"Error precondition\", when: \"($t)\", then: [\"Appropriate error response\"], real_input: \"See task specification\", expected_error: \"See task specification\"}'
    } | str join ',\n      ')
    $"    [\n      ($lines)\n    ]"
  } else {
  ...
  let e2e_cue = $"    pipeline_test: {{
      ...
      setup: {{}}
      execute: {{
        command: \"See implementation_tasks phase_4\"
      }}
      verify: {{
        exit_code: 0
      }}
  ...
  let failure_cue = $"    failure_modes: [
      {symptom: \"Tests fail\", likely_cause: \"Implementation does not match specification\", where_to_look: [{{file: \"See related_files\", what_to_check: \"Compare with contracts postconditions\"}}], fix_pattern: \"Re-read task specification and implement to match\"}
    ]"
  ...
  let read_before_write_cue = if ($related_files | length) > 0 {
    ...
    $'{file: \"See research_requirements files_to_read\", must_read_first: true, key_sections_to_understand: [\"All existing implementations\"]}'
  }
...
def generate-bead-id [] {
  # Generate unique bead ID in format: <repo-name>-TIMESTAMP-RANDOM
  ...
  $"($context.repo_name)-($timestamp)-($random_suffix)"
}
...
def main [] {
  ...
  print "  create        - Create bead in bd database"
