...
Holzman rules are brutal on modified production Rust and required proof obligations. They are not a license to make old unrelated repo-wide debt block bead-local delivery.
...
- `BLOCK_RELEASE`: release/critical beads must satisfy the selected global gate, usually `verify-all`.
- `DEFERRED_GLOBAL`: pre-existing unrelated workspace warnings, coverage gaps, mutation survivors, audit issues, or flaky tests are recorded as follow-up work and do not restart unrelated bead delivery.
...
Fallback preserves the same lint and tool intent. If any fallback tool or lint is unavailable, report the exact missing tool/lint as a blocker or residual risk. Do not silently drop non-nightly checks. In bead workflows, classify fallback failures against `delivery-scope.jsonl` and `baseline-report.md`; local and regression failures block, pre-existing unrelated global debt is deferred with follow-up evidence.
...
{"kind":"rule","id":"power10_zero_warnings","level":"fatal","text":"Production/source warnings, clippy findings, formatting drift, and static-analysis findings block completion when they are in delivery scope, newly introduced, required by an obligation, or release/critical global policy. Pre-existing unrelated repo-wide debt is DEFERRED_GLOBAL follow-up evidence, not bead-local failure. Test implementation style warnings are not a Rust delivery gate; test compile, execution, assertions, determinism, and mutation evidence remain mandatory."}
...
{"kind":"rule","id":"scope_aware_blocking","level":"fatal","text":"Classify Rust gate failures as BLOCK_LOCAL, BLOCK_REGRESSION, BLOCK_RELEASE, REQUIRED_OBLIGATION_FAIL, WAIVED, or DEFERRED_GLOBAL. Only local, regression, required-obligation, and release/critical global failures block bead delivery; old unrelated workspace debt must be recorded with follow-up evidence."}
