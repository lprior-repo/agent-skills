...
bd create --title="black-hat: <severity> — <concise issue>" \
  --description="File: <file:line>\nReviewer: black-hat-reviewer\nSeverity: <P0|P1>\nIssue: <what's wrong>\nFix: <what to do>\nCategory: <security|quality|holzman-rust>" \
  --type=bug -p <0-1> --json
```
...
bd create --title="red-queen: missing test coverage for <module>" \
  --description="File: <file:line>\nReviewer: red-queen\nModule: <module>\nIssue: <what's untested>\nFix: Write tests covering <specific cases>\nPriority: P1 if new code has zero tests, P2 if tests are weak" \
  --type=task -p <1-2> --json
```
...
```bash
# Create all findings from a single reviewer in one batch
for finding in "${FINDINGS[@]}"; do
  bd create --title="$finding" --description="..." --type=bug -p $priority --json
done
```

Always link back to the branch being reviewed:
```bash
bd create --title="..." --description="Branch: $BRANCH\n..." --deps discovered-from:<parent-bead> --json
```
